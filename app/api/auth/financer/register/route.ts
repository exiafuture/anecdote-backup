...
export async function POST(req: Request) {
  ...
  if (existingUserInCreator) {
    return NextResponse.json({ error: 'Username already exists in Financer' }, { status: 409 });
  }
  ...
  if (existingEmailInCreator) {
    return NextResponse.json({ error: 'Email already exists in Financer' }, { status: 409 });
  }
  ...
  try {
    ...
    const newFinancer = await prisma.financer.create({
      data: {
        username,
        email,
    ...
    await prisma.subscription.update({
      ...
      data: {
        financerId: newFinancer.id,  // Link the subscription to the newly created creator
      },
    });

    const token = jwt.sign(
      { userId: newFinancer.id, username: newFinancer.username, email: newFinancer.email },
    ...
    return NextResponse.json({ message: 'User registered successfully', 
      token,
      userId: newFinancer.id }, { status: 201 });
  } catch (error) {
